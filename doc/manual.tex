...
\end{equation}
where $f$ is the distribution function, $u$ is partical velocity, $\tau=\mu/p$ is particle collision time, $\mu$ is the dynamic viscosity, $p$ is the pressure and $f^+$ is the modified equilibrium distribution function.
...
\end{equation} 
where $\mu_{ref}$ is the reference visocosity and $T_{ref}$ is the reference temperature, $\omega$ is the index related to HS or VHS model.

The collision term meets the requirement of conservative constriant or compability condition
\begin{equation} 
    \int (f^+-f)\psi d\Xi=0
\end{equation} 
where $\psi=(1,u,1/2(u^2+\xi^2))^T$ is the collision invarients and $d\Xi=dud\xi$
...
\begin{equation} 
    ...
    %W_i^{n+1} = W_i^n+\frac{1}{\Delta x}\int\psi({\bf F}_{i-1/2}-{\bf F}_{i+1/2}){\rm d\Xi}
...
The overview flow chart of the solution algrithm in one iteration is shown in Figure. ~\ref{pic:solution}
...
The Maxwellian distribution around the interface in $f^+$ is approxmiated by Taylor expansion
\begin{equation} 
...
$g_0$ or $W_0$ can be obtained by applying the compability condition at $x=0,t=0$
...
The time derivative of $W$ can be calculated via the compability condition
...
\begin{figure}[htb!]
    ...
    \begin{tikzpicture}[thick]
        ...
        (at) edge (tt);
        \path[line] node[block,below of=tt] (gf) {calculate flux of conservative variables related to $g_0$}
        (tt) edge (gf);
        \path[line] node[block,below of=gf] (ff) {calculate the remaining flux}
        (gf) edge (ff);
    \end{tikzpicture}
...
Take $h$ as example. Since we take value from $h_{i+1/2,k}^L$ only if $u_k\geqslant 0$ and take value from $h_{i+1/2,k}^R$ only if $u_k <0$ (see Eq. \ref{eq:csolution_final}), there is no need to store the left and right values seperately. 
...
The macroscopic slope is approximiated by
...
\begin{figure}[htb!]
    \centering
    \begin{tikzpicture}[thick]
        \path[line] node[block] (ww) {calculate $W^{n+1}$ by Eq. \ref{eq:conservative_discrete}};
        \path[line] (ww) ++(0,-1.5) coordinate node[block] (gg) {calculate\\ $H^{n},B^{n},\tau^{n}$ at $t=t^n$ \\ $H^{n+1},B^{n+1},\tau^{n+1}$ at $t=t^{n+1}$}
        (ww) edge (gg);
        \path[line] (gg) ++(0,-1.5) coordinate node[block] (qf) {calculate heat flux ${\bf q}$ with $h^n,b^n,W^n$}
        (gg) edge (qf);
        \path[line] (qf) ++(0,-1.5) coordinate node[block] (gp) {calculate\\ $h^{+(n)},b^{+(n)}$ with ${\bf q},H^n,B^n,W^n$\\ $h^{+(n+1)},b^{+(n+1)}$ with ${\bf q},H^{n+1},B^{n+1},W^{n+1}$}
        (qf) edge (gp);
        \path[line] (gp) ++(0,-1.5) coordinate node[block] (ff) {calculate $h^{n+1}$ and $b^{n+1}$}
        (gp) edge (ff);
    \end{tikzpicture}
...
Only isothermal wall boundary condition with complete accomodation is discussed. Assuming left wall($x=1/2$)
...
Second, calculate the density at the wall with the conditon that no particle penetrating the wall
...
where $g_w,\rho_w,\lambda_w,U_w$ are the the variables at the wall.
...
\begin{appendices}
    ...
    The interation limits of $<u^n>$ is from $-\infty$ to $\infty$
    $$
    \begin{aligned}
        <u^0> &= 1\\
        <u^1> &= U\\
