...
\begin{itemize}
    ...
    \item Bash shell
    \item Latex: only for compilation of the manual. It requires hyperref, parskip, amsmath, amssymb, fullpage and appendix packages, and also bibtex, dvips and ps2pdf.
\end{itemize}
...
\begin{enumerate}
    \item Compile both UGKS1D and UGKS2D with openmp and ifort\\  make
    \item Only compile UGKS1D\\ make 1D
    \item Only compile UGKS2D\\ make 2D
    \item Compile both UGKS1D and UGKS2D, but \textbf{WITHOUT} openmp, and \textbf{WITH} gfortran\\ make OMP=no FC=gfortran
    \item Compile the manual\\ make manual
    \item Clean the compilation\\ make clean
\end{enumerate}
...
\end{itemize}
Both files are in tecplot format.
...
git clone https://github.com/lainme/UGKS.git
